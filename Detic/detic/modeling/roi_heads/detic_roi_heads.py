...
class DeticCascadeROIHeads(CascadeROIHeads):
    ...
    def _forward_box(self, features, proposals, targets=None, 
        ...
        if self.training:
            ...
            if self.one_class_per_proposal:
                # [:, None] 等价于 unsqueeze(1)
                # s == …… 相当于在前C列（不算最后一列背景列）上最大的值所在的置1 其余置0
                # s *  (s == ……)相当于每行最大值还在，其余的都置0
            ...
            h, w = (proposals[0].image_size)
            box_tensor = boxes[0].clone().detach()
            self.save_proposal_info(h, w, box_tensor, proposals[0].objectness_logits)

            # box_tensor[:, 0::2] *= scale_x
            # box_tensor[:, 1::2] *= scale_y
            # draw_boxes(image_path='/Users/shen/PycharmProjects/flicker30k/flickr30k-images/7809919056.jpg',
            #            box_list=box_tensor.tolist()[:20],word_list=[None]*len(box_tensor.tolist()[:20]), output_path='/Users/shen/PycharmProjects/Detic/Detic/output/out2.jpg')
    ...
    def save_proposal_info(self, h, w, box_tensor, scores):
        self.proposal_info = [h, w, box_tensor, scores]

    def get_proposal_info(self):
        return self.proposal_info